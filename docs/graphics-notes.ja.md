...
VRMキャラクターと浮遊キューブは、フラグメントシェーダー内で小さなtoonライティングパスを使います。
これは完全なMToon実装ではありませんが、ワールド法線、ライト位置、half-Lambertライティング、shade colorのブレンドを使って、モデルやキューブの明るい面と暗い面を読み取りやすくしています。
