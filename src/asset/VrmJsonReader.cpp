...
namespace matrixalchemy::asset::gltf
{
    namespace
    {
        ...
        std::optional<std::string_view> findMaterialPropertyObject(std::string_view vrmJson, std::string_view materialName)
        {
            ...
            while (true)
            {
                ...
                const std::size_t floatPropertiesKey = vrmJson.rfind("\"floatProperties\"", nameKey);
                const std::size_t objectBegin = floatPropertiesKey == std::string_view::npos ? std::string_view::npos : vrmJson.rfind('{', floatPropertiesKey);
        ...
        std::optional<glm::vec3> readVrmMaterialVec3Property(std::string_view materialProperty, std::string_view propertyName)
        {
        ...
        std::optional<glm::vec4> readVrmMaterialVec4Property(std::string_view materialProperty, std::string_view propertyName)
        {
            const std::optional<std::array<float, 4>> color = readJsonVec4Property(materialProperty, propertyName);
            if (!color.has_value())
            {
    ...
    ToonMaterial readVrmToonMaterial(std::string_view vrmJson, std::string_view materialName)
    {
        ToonMaterial toonMaterial;
        const std::optional<std::string_view> materialProperty = findMaterialPropertyObject(vrmJson, materialName);
        if (!materialProperty.has_value())
        {
            return toonMaterial;
        }

        toonMaterial.shadeColor = readVrmMaterialVec3Property(*materialProperty, "_ShadeColor");
        const std::optional<glm::vec4> rimColor = readVrmMaterialVec4Property(*materialProperty, "_RimColor");
        if (rimColor.has_value())
        {
            toonMaterial.rimColor = glm::vec3(*rimColor) * rimColor->a;
        }
        toonMaterial.emissionColor = readVrmMaterialVec3Property(*materialProperty, "_EmissionColor");
        toonMaterial.shadeShift = readJsonFloatProperty(*materialProperty, "_ShadeShift");
        toonMaterial.shadeToony = readJsonFloatProperty(*materialProperty, "_ShadeToony");
        toonMaterial.rimFresnelPower = readJsonFloatProperty(*materialProperty, "_RimFresnelPower");
        toonMaterial.outlineColor = readVrmMaterialVec4Property(*materialProperty, "_OutlineColor");
